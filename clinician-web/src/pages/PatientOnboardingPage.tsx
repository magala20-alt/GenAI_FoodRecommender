...
export function PatientOnboardingPage() {
  ...
  const [incomeLevel, setIncomeLevel] = useState('Low-Middle')
  const [employmentStatus, setEmploymentStatus] = useState('Employed')
  const [smokingStatus, setSmokingStatus] = useState('Ex-smoker')
  const [alcoholUnits, setAlcoholUnits] = useState('3')
  const [activityMinutes, setActivityMinutes] = useState('90')
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        <header className="bg-white border-b border-slate-200 px-7 py-4 flex items-center justify-between">
          <div className="flex items-center gap-2 text-sm">
            <button onClick={() => navigate('/patients')} className="text-slate-400 hover:text-slate-700">← Patient List</button>
          ...
          <div className="flex items-center gap-4">
            ...
            <button onClick={() => navigate('/patients')} className="text-sm border border-slate-200 px-3 py-1.5 rounded-lg hover:bg-slate-50">× Cancel</button>
          </div>
        </header>

        <section className="flex-1 overflow-y-auto">
          {step === 1 && (
            <div className="h-full min-h-[720px] bg-white border-b border-slate-200">
              <div className="flex flex-col md:flex-row h-full min-h-[720px]">
                <OnboardingStepRail step={step} goToStep={goToStep} />

                <div className="flex-1 px-4 md:px-6 py-5 overflow-y-auto">
                    <div className="mb-4">
                      <h2 className="text-2xl font-bold text-slate-900">Personal Information</h2>
                      <p className="text-sm text-slate-500 mt-1">Enter the patient's identity, demographic, and lifestyle details.</p>
                    </div>

                    <div className="mb-5">
                      <div className="text-[10px] font-bold tracking-[0.15em] uppercase text-teal-600 mb-2 pb-1 border-b border-teal-100">Identity</div>
                      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                        ...
                        <ReadOnlyField label="Age (auto)" value={age ? `${age}` : ''} />
                        <SelectField label="Gender" value={gender} onChange={setGender} options={['Female', 'Male', 'Other']} required />
                        <SelectField label="Ethnicity" value={ethnicity} onChange={setEthnicity} options={['Black / African', 'Asian', 'Hispanic / Latino', 'White', 'Other']} />
                        <Input label="Email" value={email} onChange={setEmail} required type="email" />
                        <Input label="Phone" value={phone} onChange={setPhone} />
                        <SelectField label="Country" value={country} onChange={setCountry} options={['Ghana', 'Nigeria', 'Kenya', 'South Africa']} />
                      </div>
                    </div>

                    <div className="mb-5">
                      <div className="text-[10px] font-bold tracking-[0.15em] uppercase text-teal-600 mb-2 pb-1 border-b border-teal-100">Socioeconomic</div>
                      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                        <SelectField label="Education Level" value={educationLevel} onChange={setEducationLevel} options={['Primary', 'Secondary', 'Tertiary', 'Postgraduate']} />
                        <SelectField label="Income Level" value={incomeLevel} onChange={setIncomeLevel} options={['Low', 'Low-Middle', 'Middle', 'Upper-Middle', 'High']} />
                        <SelectField label="Employment Status" value={employmentStatus} onChange={setEmploymentStatus} options={['Employed', 'Self-employed', 'Unemployed', 'Retired', 'Student']} />
                      </div>
                    </div>

                    <div className="mb-5">
                      <div className="text-[10px] font-bold tracking-[0.15em] uppercase text-teal-600 mb-2 pb-1 border-b border-teal-100">Lifestyle</div>
                      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                        <SelectField label="Smoking Status" value={smokingStatus} onChange={setSmokingStatus} options={['Never', 'Ex-smoker', 'Current smoker']} />
                        <Input label="Alcohol (units/week)" value={alcoholUnits} onChange={setAlcoholUnits} />
                        <Input label="Activity (min/week)" value={activityMinutes} onChange={setActivityMinutes} />
                        <Input label="Diet Score (1-10)" value={dietScore} onChange={setDietScore} />
                        <Input label="Sleep (hrs/day)" value={sleepHours} onChange={setSleepHours} />
                        <Input label="Screen Time (hrs/day)" value={screenTimeHours} onChange={setScreenTimeHours} />
                      </div>
                    </div>

                    <div className="bg-slate-50 border border-slate-200 rounded-xl p-3 mb-4">
                      <p className="text-sm font-semibold text-slate-800 mb-2">Emergency Contact <span className="text-xs font-normal text-slate-500">(optional)</span></p>
                      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                    ...
                    <div className="flex justify-end gap-2">
                      <button onClick={() => navigate('/patients')} className="text-sm border border-slate-200 px-4 py-2 rounded-lg hover:bg-slate-50">× Cancel</button>
                      <button onClick={() => goToStep(2)} className="text-sm bg-teal-500 hover:bg-teal-600 text-white px-4 py-2 rounded-lg font-semibold">Next: Medical Profile →</button>
                    </div>
                </div>
              </div>
            </div>
        ...
        {step !== 1 && <footer className="border-t border-slate-200 bg-white px-7 py-4 flex items-center justify-between">
          <div>
            {step > 1 ? (
              <button onClick={() => goToStep(step - 1)} className="border border-slate-200 px-4 py-2 rounded-xl hover:bg-slate-50">← Back</button>
            ) : (
              <button onClick={() => navigate('/patients')} className="border border-slate-200 px-4 py-2 rounded-xl hover:bg-slate-50">× Cancel</button>
            )}
          </div>
...
function OnboardingStepRail({
  ...
  return (
    <div className="w-full md:w-[210px] bg-slate-900 px-4 py-6 shrink-0 flex flex-col">
      <p className="text-[10px] font-bold tracking-[0.15em] uppercase text-slate-500 mb-5">Onboarding Steps</p>
      <div className="space-y-1">
        {onboardingSteps.map(item => {
          ...
          return (
            <button key={item.id} onClick={() => goToStep(item.id)} className="w-full flex gap-3 items-start text-left">
              <div className="flex flex-col items-center">
                ...
                {item.id < onboardingSteps.length && <div className="w-0.5 h-11 bg-white/10 mt-1" />}
              </div>
              <div className="pt-1">
                <p className={clsx('text-sm', isActive || isDone ? 'font-bold text-white' : 'font-medium text-slate-400')}>{item.title}</p>
                <p className={clsx('text-xs mt-0.5', isActive || isDone ? 'text-slate-400' : 'text-slate-600')}>{item.subtitle}</p>
              </div>
            </button>
          )
        })}
      ...
      <div className="mt-auto pt-4 border-t border-slate-800">
        <p className="text-xs leading-5 text-slate-500">Patient receives a welcome email with login credentials after step 4.</p>
      </div>
    </div>
  )
}
